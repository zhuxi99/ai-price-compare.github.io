...
3. 双击桌面上的“AI价格比对－一键发布”，等待终端显示发布成功。
...
脚本会从 `~/下载`（或 `~/Downloads`）自动选择最新的有效非空备份，将数据写入 `.surge/index.html` 和 GitHub Pages 使用的根目录 `index.html`，并部署到固定 Surge 域名。空备份和格式错误的文件会被跳过。
...
更新 GitHub Pages 时，将新生成的 `index.html` 提交并推送到 `main` 分支即可。
